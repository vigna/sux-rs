...
macro_rules! call_with_sorted {
    ...
    // For front_coded_list::store with one type parameter and one const generic
    ($unsorted:expr, front_coded_list::store_str, $($args:expr),*) => {
        if $unsorted {
            front_coded_list::store_str::<_,_, false>($($args),*)
        } else {
            front_coded_list::store_str::<_,_,  true>($($args),*)
        }
    };
}

#[derive(Parser, Debug)]
#[command(about = "Builds a front-coded list starting from a list of UTF-8 encoded strings.", long_about = None)]
struct Args {
    ...
    source: String,
    /// A name for the ε-serde serialized front-coded list.
    dest: String,
...
fn compress<R: BufRead, const SORTED: bool>(
    ...
    let mut fclb = FrontCodedListBuilder::<str, SORTED>::new(ratio);
    ...
    unsafe {
        fcl.serialize(&mut dst_file)
            .expect("Cannot serialize front-coded list")
    };
    Ok(())
}
