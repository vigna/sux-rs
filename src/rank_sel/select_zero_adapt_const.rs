    ...
    unsafe fn select_zero_unchecked(&self, rank: usize) -> usize {
        unsafe {
            ...
            if inventory_rank.is_u16_span() {
                ...
                return self.bits.select_zero_hinted::<{ usize::MAX }>(
                    rank,
                    hint_pos,
                    rank - residual,
                );
...
> crate::rkyv_view::ToNative
    for ArchivedSelectZeroAdaptConst<
        B,
        Box<[W]>,
...
/// Lazy views of an archived selection structure for zeros.
///
/// Only [`select_zero_unchecked`](SelectZeroUnchecked::select_zero_unchecked) reads the inventory and the spill; in particular
/// selecting a one never dereferences the pointers of the zeros, because
...
const _: () = {
    ...
    impl<'a, B: rkyv::Archive, const A: usize, const C: usize> Words<'a>
        for Lazy<'a, ArchivedSelectZeroAdaptConst<B, Box<[usize]>, A, C>>
    where
    ...
    impl<B: rkyv::Archive, const A: usize, const C: usize> Backend
        for Lazy<'_, ArchivedSelectZeroAdaptConst<B, Box<[usize]>, A, C>>
    {
        type Word = usize;
    }
    ...
    impl<'a, B: rkyv::Archive, const A: usize, const C: usize> SelectUnchecked
        for Lazy<'a, ArchivedSelectZeroAdaptConst<B, Box<[usize]>, A, C>>
    where
        Lazy<'a, B::Archived>: SelectUnchecked,
    {
        #[inline(always)]
        unsafe fn select_unchecked(&self, rank: usize) -> usize {
            unsafe { Lazy(&self.0.bits).select_unchecked(rank) }
        }
    }

    impl<'a, B: rkyv::Archive, const A: usize, const C: usize> SelectZeroUnchecked
        for Lazy<'a, ArchivedSelectZeroAdaptConst<B, Box<[usize]>, A, C>>
    where
        Lazy<'a, B::Archived>:
            Backend<Word = usize> + AsRef<[usize]> + BitLength + SelectZeroHinted,
    {
        #[inline(always)]
        unsafe fn select_zero_unchecked(&self, rank: usize) -> usize {
            // The inventory and the spill are the only pointers this method
            // needs: the bits stay lazy, and `select_zero_hinted` resolves them.
            let view = SelectZeroAdaptConst::<Lazy<'a, B::Archived>, &'a [usize], A, C> {
            ...
            unsafe { view.select_zero_unchecked(rank) }
        }
    }
};
