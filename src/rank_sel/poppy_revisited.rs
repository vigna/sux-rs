...
#[derive(Epserde, Debug, Clone, MemDbg, MemSize)]
pub struct PoppyRevisited<
    B: RankHinted<HINT_BIT_SIZE> + Rank + BitCount + AsRef<[usize]> = BitVec,
    const HINT_BIT_SIZE: usize = 64,
> {
    pub(super) bits: B,
    pub(super) counts: CountsForPoppy,
    pub(super) num_ones: u64,
}

impl<const HINT_BIT_SIZE: usize> PoppyRevisited<BitVec, HINT_BIT_SIZE> {
    pub fn new(bits: BitVec) -> Self {
        ...
        let mut counts = CountsForPoppy::new(num_major_counts, num_counts);
...
impl<
    ...
    > BitLength for PoppyRevisited<B, HINT_BIT_SIZE>
{
    fn len(&self) -> usize {
        self.bits.len()
    }
...
impl<
        B: RankHinted<HINT_BIT_SIZE> + Rank + BitCount + AsRef<[usize]>,
        const HINT_BIT_SIZE: usize,
    > BitCount for PoppyRevisited<B, HINT_BIT_SIZE>
{
    fn count(&self) -> usize {
...
impl<
        B: RankHinted<HINT_BIT_SIZE> + Rank + BitCount + AsRef<[usize]>,
        const HINT_BIT_SIZE: usize,
    > Rank for PoppyRevisited<B, HINT_BIT_SIZE>
{
...
mod test_poppy_rev {
    ...
    fn test_poppy_rev() {
        ...
        for len in lens {
            ...
            let poppy_rev: PoppyRevisited = PoppyRevisited::new(bits.clone());
            ...
            for i in 0..bits.len() {
                assert_eq!(poppy_rev.rank(i), ranks[i],);
            }
            assert_eq!(poppy_rev.rank(bits.len() + 1), bits.count_ones());
        }
    }
    ...
    fn test_last() {
        ...
        let poppy_rev: PoppyRevisited = PoppyRevisited::new(bits);

        assert_eq!(poppy_rev.rank(poppy_rev.len()), poppy_rev.bits.count());
    }
}
