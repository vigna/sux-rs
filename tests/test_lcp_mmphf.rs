...
fn test_small() -> Result<()> {
    ...
    keys.sort();
    let func = LcpMmphfStr::<[u64; 2], FuseLge3Shards>::new(
        keys_lender(&keys),
        keys.len(),
        no_logging![],
    )?;
...
fn test_single() -> Result<()> {
    let keys = vec!["hello"];
    let func = LcpMmphfStr::<[u64; 2], FuseLge3Shards>::new(keys_lender(&keys), 1, no_logging![])?;
...
fn test_two_keys() -> Result<()> {
    ...
    let func = LcpMmphfStr::<[u64; 2], FuseLge3Shards>::new(keys_lender(&keys), 2, no_logging![])?;
...
fn test_unsorted_error() {
    ...
    let result = LcpMmphfStr::<[u64; 2], FuseLge3Shards>::new(keys_lender(&keys), 2, no_logging![]);
...
fn test_small_u64() -> Result<()> {
    ...
    let func = LcpMmphfInt::<_, [u64; 2], FuseLge3Shards>::new(
        FromSlice::new(&keys),
        keys.len(),
        no_logging![],
    )?;
...
fn test_single_u64() -> Result<()> {
    ...
    let func =
        LcpMmphfInt::<_, [u64; 2], FuseLge3Shards>::new(FromSlice::new(&keys), 1, no_logging![])?;
...
fn test_two_u64() -> Result<()> {
    ...
    let func =
        LcpMmphfInt::<_, [u64; 2], FuseLge3Shards>::new(FromSlice::new(&keys), 2, no_logging![])?;
...
fn test_monotone_1000_u64() -> Result<()> {
    ...
    let func =
        LcpMmphfInt::<_, [u64; 2], FuseLge3Shards>::new(FromSlice::new(&keys), n, no_logging![])?;
...
fn test_unsorted_error_u64() {
    ...
    let result =
        LcpMmphfInt::<_, [u64; 2], FuseLge3Shards>::new(FromSlice::new(&keys), 3, no_logging![]);
    assert!(result.is_err());
}
...
fn test_empty_str() -> Result<()> {
    ...
    let func = LcpMmphfStr::<[u64; 2], FuseLge3Shards>::new(keys_lender(&keys), 0, no_logging![])?;
...
fn test_empty_u64() -> Result<()> {
    ...
    let func =
        LcpMmphfInt::<_, [u64; 2], FuseLge3Shards>::new(FromSlice::new(&keys), 0, no_logging![])?;
    assert_eq!(func.len(), 0);
    assert!(func.is_empty());
    Ok(())
}
...
fn test_signed_i64_random_1000() -> Result<()> {
    ...
    let func: LcpMmphfInt<i64> = LcpMmphfInt::new(FromSlice::new(&keys), n, no_logging![])?;
