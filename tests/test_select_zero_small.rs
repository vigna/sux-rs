...
fn test_empty() {
    ...
    let select = SelectZeroSmall::<2, 9, _>::new(RankSmall::<64, 2, 9, _>::new(bits.clone()));
...
fn test_ones() {
    ...
    let select = SelectZeroSmall::<2, 9, _>::new(RankSmall::<64, 2, 9, _>::new(bits));
...
fn test_with_inv_extreme_span() {
    ...
    let sel = SelectZeroSmall::<2, 9, _>::with_inv(RankSmall::<64, 2, 9, _>::new(bits), usize::MAX);
    for k in 0..64 {
        assert_eq!(sel.select_zero(k), Some(k));
    }
}
