...
recorded, e.g.
    ...
    {"ef_rank_unchecked/rkyv/1G_l=8": {"iters": [...], "times": [...], ...}}

Dividing `times` by `iters` elementwise gives nanoseconds per operation.
"""
...
def main():
    ...
    ap.add_argument("tree", nargs="?", default="target/criterion",
                    help="the criterion directory (default: %(default)s)")
    ap.add_argument("--baseline", default="full",
                    help="which saved baseline to read (default: %(default)s)")
    ap.add_argument("-o", "--out", default="samples.json",
